...
$(document).ready(function() {
				...
				if(tes > 0 && opsi_extra.total > 0){
					...
					var b = opsi_extra.total - tes ;
						...
						$.ajax({
							...
							,data:{'mode':'wkwkwkw','controller':helmi.controller,'list':JSON.stringify(opsi_extra.list),'bayar':tes,'kembali':kembalian,total:opsi_extra.total},
	...
	$( "#transaksi input#nama_barang" ).autocomplete({
		...
		select:function(event, ui) {
				...
				$('#transaksi div.profile-box-header').html( ( (typeof url.gambar_fol !== 'undefined' && url.gambar_fol)? '<img class="barang-img img-responsive center-block" src="'+helmi.asset+url.gambar_fol+url.gambar_fil +'" /><h2>' : '' ) + url.label +'</h2><div class="job-position">'+ url.tipe_k +'</div>');
				$('#transaksi div.profile-box-footer').html('<a href="#"><span class="value">'+ url.jumlah +'</span> <span class="label">Stock</span> </a><a href="#"><span class="value">'+ url.modal +'</span> <span class="label">Harga</span> </a><a href="#"><span class="value">'+ url.satuan_k +'</span> <span class="label">Satuan</span> </a><a href="#"><span class="value">'+ url.ukuran_k +'</span> <span class="label">Ukuran</span> </a><a href="#"><span class="value">'+ url.supplier_n +'</span> <span class="label">Supplier</span> </a>');
	...
	$('#transaksi input#jumlahbarang').keypress(function(e){
		if(e.keyCode == 13 && (typeof opsi_extra.select !== 'undefined') && opsi_extra.select != null ){
			...
			var _harga = opsi_extra.select.modal,_nama = opsi_extra.select.nama;
			...
			var b = {};
			b.total_harga = _total_blnja ; b.id_barang = opsi_extra.select.id_barang ; b.harga = opsi_extra.select.modal ; b.total = jml_belanja ;
