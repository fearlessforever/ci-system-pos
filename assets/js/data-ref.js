...
$(document).ready(function(){
	$(document).on('click','[data-tombol]',function(e){
		...
		switch($(this).attr('data-tombol')){
			case 'tambah' :
				...
				$('#modalnya-bro .modal-header').html('<button type="button" class="close" data-dismiss="modal">&times;</button><h4 class="modal-title">Tambah Ref / Nomor Akun</h4>');
				$('#modalnya-bro .modal-body').html('<form role="form" perintah="tambah"><div class="form-inline form-inline-box"><div class="form-group"> <label >Ref / Nomor Akun</label> <input class="form-control" placeholder=" Misal 101" type="text" name="code"> <label class="btn btn-danger" data-tombol="check-duplicate" data-s="code"> Check Duplicate <i class="fa fa-history"></i></label> </div></div><div class="form-group"> <label > Nama Akun </label> <input class="form-control" placeholder="Nama Akun misal Kas,Piutang Usaha" type="text" name="ket"> </div> <input type="hidden" name="mode" value="tambah"> <input type="hidden" name="controller" value="'+helmi.controller+'"><input type="hidden" name="check-duplicate" value=""> </form>');
	...
	function buat_tabel(e){
		...
		return '<table id="table-gwa-crud" class="table table-hover"><thead><tr><th>No</th><th>Ref / Nomor Akun</th><th>Nama Akun</th><th>Action</th></tr></thead><tbody>'+c+'</tbody></table>';
	}
});