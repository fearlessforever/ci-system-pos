...
$(document).ready(function(){
	$(document).on('click','[data-tombol]',function(e){
		...
		switch($(this).attr('data-tombol')){
			case 'tambah' :
				...
				$('#modalnya-bro .modal-header').html('<button type="button" class="close" data-dismiss="modal">&times;</button><h4 class="modal-title">Tambah Tipe</h4>');
				$('#modalnya-bro .modal-body').html('<form role="form" perintah="tambah"><div class="form-inline form-inline-box"><div class="form-group"> <label >Code Tipe</label> <input class="form-control" placeholder="Code nya Misal 1" type="text" name="code"> <label class="btn btn-danger" data-tombol="check-duplicate" data-s="code"> Check Duplicate <i class="fa fa-history"></i></label> </div></div><div class="form-group"> <label >Keterangan Tipe</label> <input class="form-control" placeholder="Tipe Misal Minuman/ Makanan" type="text" name="ket"> </div> <input type="hidden" name="mode" value="tambah"> <input type="hidden" name="controller" value="'+helmi.controller+'"><input type="hidden" name="check-duplicate" value=""> </form>');
			...
			case 'hapus':
				...
				if(typeof a.data('code') !== 'undefined' ){
					...
					$('#modalnya-bro .modal-body').html('<p>Peringatan : Menghapus Code Tipe yang telah di gunakan di List Barang sangat tidak dianjurkan<br/>Jika Anda tetap ingin menghapus data ini silahkan klik Hapus untuk konfirmasi hapus<br><br>Anda akan menghapus Tipe dengan code berikut :<h4 class="label label-info">'+a.data('code')+'</h4> </p><form perintah="hapus"><input type="hidden" name="code" value="'+a.data('code')+'"><input type="hidden" name="mode" value="hapus"><input type="hidden" value="'+helmi.controller+'" name="controller" /></form>');
	...
	function buat_tabel(e){
		...
		return '<table id="table-gwa-crud" class="table table-hover "><thead><tr><th>No</th><th>Code Tipe</th><th>Keterangan</th><th>Action</th></tr></thead><tbody>'+c+'</tbody></table>';
	}
});