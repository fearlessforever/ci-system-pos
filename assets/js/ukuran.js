...
opsi_extra.edit = function(){
	...
	if( aa.length > 0 && typeof opsi_extra.table_c !== 'undefined' && typeof opsi_extra.table_b !== 'undefined'){
		...
		var a = '<tr data-code="'+b.code+'" data-detail=\''+ c +'\'><td>'+b.code+'</td><td>'+b.ket+'</td><td>'+( (b.code != 1 ) ?'<span data-tombol="hapus" class="btn btn-danger">HAPUS</span> &nbsp; <span data-tombol="edit" class="btn btn-info">EDIT</span> ' : '-' )+'</td></tr>'; 
	...
	$(document).on('click','[data-tombol]',function(e){
		...
		switch($(this).attr('data-tombol')){
			case 'tambah' :
				...
				$('#modalnya-bro .modal-header').html('<button type="button" class="close" data-dismiss="modal">&times;</button><h4 class="modal-title">Tambah Ukuran</h4>');
				$('#modalnya-bro .modal-body').html('<form role="form" perintah="tambah"><div class="form-inline form-inline-box"><div class="form-group"> <label >Code Ukuran</label> <input class="form-control" placeholder="Code nya Misal 1" type="text" name="code"><label class="btn btn-danger" data-tombol="check-duplicate" data-s="code"> Check Duplicate <i class="fa fa-history"></i></label> </div></div><div class="form-group"> <label >Keterangan Ukuran</label> <input class="form-control" placeholder="Ukuran Misal XL" type="text" name="ket"> </div> <input type="hidden" name="mode" value="tambah"> <input type="hidden" name="controller" value="'+helmi.controller+'"><input type="hidden" name="check-duplicate" value=""> </form>');
			...
			case 'hapus':
				...
				if(typeof a.data('code') !== 'undefined' ){
					...
					$('#modalnya-bro .modal-body').html('<p>Peringatan : Menghapus Code Ukuran yang telah di gunakan di List Barang sangat tidak dianjurkan<br/>Jika Anda tetap ingin menghapus data ini silahkan klik Hapus untuk konfirmasi hapus<br><br>Anda akan menghapus Ukuran dengan code berikut :<h4 class="label label-info">'+a.data('code')+'</h4> </p><form perintah="hapus"><input type="hidden" name="code" value="'+a.data('code')+'"><input type="hidden" name="mode" value="hapus"><input type="hidden" value="'+helmi.controller+'" name="controller" /></form>');
	...
	function buat_tabel(e){
		...
		if(e instanceof Array){
			$.each(e,function(k,val){
				...
				c += '<tr data-code="'+val.code+'" data-detail=\''+b+'\'><td>'+val.code+'</td><td>'+val.ket+'</td><td>'+( (val.code != 1 ) ?'<span class="btn btn-danger" data-tombol="hapus">HAPUS</span> &nbsp; <span class="btn btn-info" data-tombol="edit">EDIT</span>' : '-' )+'</td></tr>';
			});
		}else{
			c ='<tr><td><h2>TIDAK ADA DATA</h2></td><td></td><td></td></tr>';
		}
		return '<table id="table-gwa-crud" class="table"><thead><tr><th>Code Ukuran</th><th>Keterangan</th><th>Action</th></tr></thead><tbody>'+c+'</tbody></table>';
	}
});