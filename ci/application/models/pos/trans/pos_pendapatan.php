...
class Pos_pendapatan extends CI_Model {
	...
	public function run(){
		...
		if($check){
			...
			switch($mode){
				case 'okelah': self::__pembayaran(); break;
				default: break;
			}
	...
	private function __pembayaran(){
		...
		$error = false;
		$this->db->trans_start();
			$code = $this->db->query("SELECT kode_auto('penjualan',true) as code ")->row_array();
			$data['id_transaksi'] = $code['code'];
			$this->db->query("INSERT INTO ".DB_KODE."transaksi(bayar,kembali,total_h,detail,userid,id_transaksi,tanggal,waktu) VALUES(?,?,?,?,?,?,current_date(),now() ) ",$data);
			
			// SAVE ke DB model Prepare
			mysqli_report( MYSQLI_REPORT_ERROR | MYSQLI_REPORT_STRICT );
			if($stmt = mysqli_prepare($this->db->conn_id , "INSERT INTO ".DB_KODE."transaksi_detail(id_transaksi,tanggal,nmr,id_barang,jumlah,harga,userid) VALUES(?,CURRENT_DATE(),?,?,?,?,?) ") ){
		...
		if($this->db->trans_status() === FALSE){// Check if transaction result successful
		   ...
		   $this->db->query("INSERT INTO ".DB_KODE."pengguna_ext(userid,nama,isi) VALUES(?,?,1) ON DUPLICATE KEY UPDATE isi=isi+1 ",array($this->session->_user['userid'] , 'trans_pendapatan') );
