...
class Pos_satuan extends CI_Model {
	...
	private function __duplicate(){
		...
		$core = $this->db->query("SELECT id_satuan FROM ".DB_KODE."data_satuan WHERE id_satuan=? LIMIT 1",array('S'.$code))->row_array();
		if(isset($core['id_satuan'])){
	...
	private function __edit(){
		...
		$this->db->where('id_satuan',$code )->limit(1);
		
		$this->db->update(DB_KODE .'data_satuan',$data);
	...
	private function __tambah(){
		...
		$code = 'S'.$code;
		$data = array(
			'id_satuan'=>$code ,'ket'=>$keterangan
		);
		
		$this->db->query("INSERT IGNORE INTO ".DB_KODE."data_satuan(".implode(',',array_keys($data)).") VALUES(?".str_repeat(',?',(count($data)-1) ) .") " ,$data);
		$core = $this->db->affected_rows();
		if($core){
			self::$_hasil=array('berhasil'=>'Data Berhasil Disimpan');
			self::$_hasil['baru']=array('code'=> $code ,'ket'=> $keterangan );
	...
	private function __hapus(){
		...
		$core = $this->db->query("DELETE FROM ".DB_KODE."data_satuan WHERE id_satuan=? LIMIT 1" ,array($code) );
		if($core){
	...
	private function __view(){
		$core = $this->db->query("SELECT id_satuan as code,ket FROM ".DB_KODE."data_satuan LIMIT 1000")->result_array();
		if(isset($core[0]) && is_array($core)){
			$num = $this->db->count_all_results(DB_KODE.'data_satuan');
