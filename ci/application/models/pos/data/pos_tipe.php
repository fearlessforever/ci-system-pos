...
class Pos_tipe extends CI_Model {
	...
	private function __duplicate(){
		...
		$core = $this->db->query("SELECT id_tipe FROM ".DB_KODE."data_tipe WHERE id_tipe=? LIMIT 1",array('T'.$code))->row_array();
		if(isset($core['id_tipe'])){
	...
	private function __tambah(){
		...
		$code = 'T'.$code;
		$data = array(
			'id_tipe'=> $code,'ket'=>$keterangan
		);
		
		$this->db->query("INSERT IGNORE INTO ".DB_KODE."data_tipe(".implode(',',array_keys($data)).") VALUES(?".str_repeat(',?',(count($data)-1) ) .") " ,$data);
		$core = $this->db->affected_rows();
		if($core){
			self::$_hasil=array('berhasil'=>'Data Berhasil Disimpan');
			self::$_hasil['baru']=array(
	...
	private function __hapus(){
		...
		$core = $this->db->query("DELETE FROM ".DB_KODE."data_tipe WHERE id_tipe=? LIMIT 1" ,array($code) );
		if($core){
	...
	private function __view(){
		$core = $this->db->query("SELECT id_tipe as code,ket FROM ".DB_KODE."data_tipe LIMIT 1000")->result_array();
		if(isset($core[0]) && is_array($core)){
			$num = $this->db->count_all_results(DB_KODE.'data_tipe');
